...
# -------------------------
# Load clinical + image features
# -------------------------
def load_data(clinical_path, image_feature_path):
    df_clinical = pd.read_csv(clinical_path)
    df_image = pd.read_csv(image_feature_path)

    # Merge on PatientID
    df = df_clinical.merge(df_image, how="inner", left_on="PatientID", right_on="patient_id")
...
def main():
    ...
    clinical_file = "data/clinical2_with_rna.csv"
    image_file = "outputs/image_features_dataset2.csv"

    X, y = load_data(clinical_file, image_file)
    ...
    pd.DataFrame([{
        "model": "ModelC_Clinical2Image_RF",
        "auc": round(auc, 4),
        "f1_score": round(f1, 4),
        ...
        "max_depth": 5,
        "min_samples_leaf": 5
    }]).to_csv("results/modelC_clinical2image_results.csv", index=False)
